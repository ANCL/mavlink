...
#define MAVLINK_MSG_ID_UALBERTA_ATTITUDE_LEN 52
#define MAVLINK_MSG_ID_227_LEN 52
...
#define MAVLINK_MESSAGE_INFO_UALBERTA_ATTITUDE { \
	...
	{  { "nav_euler", NULL, MAVLINK_TYPE_FLOAT, 3, 0, offsetof(mavlink_ualberta_attitude_t, nav_euler) }, \
         ...
         { "time_boot_ms", NULL, MAVLINK_TYPE_UINT32_T, 0, 48, offsetof(mavlink_ualberta_attitude_t, time_boot_ms) }, \
         } \
}
...
static inline uint16_t mavlink_msg_ualberta_attitude_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       const float *nav_euler, const float *nav_euler_rate, const float *ahrs_euler, const float *ahrs_euler_rate, uint32_t time_boot_ms)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, 52, 151);
}
						           ...
						           const float *nav_euler,const float *nav_euler_rate,const float *ahrs_euler,const float *ahrs_euler_rate,uint32_t time_boot_ms)
{
	...
	msg->msgid = MAVLINK_MSG_ID_UALBERTA_ATTITUDE;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 52, 151);
}
...
{
	return mavlink_msg_ualberta_attitude_pack(system_id, component_id, msg, ualberta_attitude->nav_euler, ualberta_attitude->nav_euler_rate, ualberta_attitude->ahrs_euler, ualberta_attitude->ahrs_euler_rate, ualberta_attitude->time_boot_ms);
}
...
static inline void mavlink_msg_ualberta_attitude_send(mavlink_channel_t chan, const float *nav_euler, const float *nav_euler_rate, const float *ahrs_euler, const float *ahrs_euler_rate, uint32_t time_boot_ms)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[52];
	_mav_put_uint32_t(buf, 48, time_boot_ms);
	...
	_mav_put_float_array(buf, 24, ahrs_euler, 3);
	_mav_put_float_array(buf, 36, ahrs_euler_rate, 3);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_UALBERTA_ATTITUDE, buf, 52, 151);
#else
	mavlink_ualberta_attitude_t packet;
	packet.time_boot_ms = time_boot_ms;
	mav_array_memcpy(packet.nav_euler, nav_euler, sizeof(float)*3);
	...
	mav_array_memcpy(packet.ahrs_euler, ahrs_euler, sizeof(float)*3);
	mav_array_memcpy(packet.ahrs_euler_rate, ahrs_euler_rate, sizeof(float)*3);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_UALBERTA_ATTITUDE, (const char *)&packet, 52, 151);
#endif
}
...
#else
	memcpy(ualberta_attitude, _MAV_PAYLOAD(msg), 52);
#endif
}
