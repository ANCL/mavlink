...
#define MAVLINK_MSG_ID_UALBERTA_POSITION_LEN 48
#define MAVLINK_MSG_ID_224_LEN 48
...
static inline uint16_t mavlink_msg_ualberta_position_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       const float *llh_pos, const float *ned_pos, const float *ned_vel, const float *ned_origin)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, 48, 99);
}
						           ...
						           const float *llh_pos,const float *ned_pos,const float *ned_vel,const float *ned_origin)
{
	...
	msg->msgid = MAVLINK_MSG_ID_UALBERTA_POSITION;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 48, 99);
}
...
{
	return mavlink_msg_ualberta_position_pack(system_id, component_id, msg, ualberta_position->llh_pos, ualberta_position->ned_pos, ualberta_position->ned_vel, ualberta_position->ned_origin);
}
...
static inline void mavlink_msg_ualberta_position_send(mavlink_channel_t chan, const float *llh_pos, const float *ned_pos, const float *ned_vel, const float *ned_origin)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[48];
	...
	_mav_put_float_array(buf, 24, ned_vel, 3);
	_mav_put_float_array(buf, 36, ned_origin, 3);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_UALBERTA_POSITION, buf, 48, 99);
#else
	mavlink_ualberta_position_t packet;
	...
	mav_array_memcpy(packet.ned_vel, ned_vel, sizeof(float)*3);
	mav_array_memcpy(packet.ned_origin, ned_origin, sizeof(float)*3);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_UALBERTA_POSITION, (const char *)&packet, 48, 99);
#endif
}
...
#else
	memcpy(ualberta_position, _MAV_PAYLOAD(msg), 48);
#endif
}
